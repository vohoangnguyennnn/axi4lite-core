...
module axi4lite_slave_adapter #(
  ...
  initial begin : p_check_interface_widths
    if (($bits(s_axi.AWADDR) != int'(ADDR_WIDTH)) ||
        ($bits(s_axi.ARADDR) != int'(ADDR_WIDTH)) ||
    ...
    if (($bits(s_axi.WDATA) != int'(DATA_WIDTH)) ||
        ($bits(s_axi.RDATA) != int'(DATA_WIDTH)) ||
    ...
    if (($bits(s_axi.WSTRB) != int'(STRB_WIDTH)) ||
