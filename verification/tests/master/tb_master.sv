// Master adapter bench: random native requester -> axi4lite_master_adapter
// -> random AXI4-Lite slave. The scoreboard compares native transactions
// with the AXI transactions they produce.
module tb_master
...
#(
  ...
  axi4lite_master_adapter #(.ADDR_WIDTH(32), .DATA_WIDTH(DATA_WIDTH)) dut (
  ...
  axi4lite_slave_model    u_slave     (.axi(axi));
  ...
  sb_path u_scoreboard (
    ...
    .b_wr_req_evt(a_wr_req_evt), .b_wr_req_txn(a_wr_req_txn),
    .b_wr_rsp_evt(a_wr_rsp_evt), .b_wr_rsp_txn(a_wr_rsp_txn),
    .b_rd_req_evt(a_rd_req_evt), .b_rd_req_txn(a_rd_req_txn),
    .b_rd_rsp_evt(a_rd_rsp_evt), .b_rd_rsp_txn(a_rd_rsp_txn),
    .empty(sb_empty), .wr_matched(wr_matched), .rd_matched(rd_matched)
  );

  tb_end_check #(.NAME("master")) u_end_check (
